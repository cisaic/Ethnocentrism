...
;;;new in this version
;;;  mac format for file paths
;;;  don't plot strategies in run1
;;;  default separator is tab in lists->file
;;;  .txt suffix for saved files
...
(defun run1 (ncycles lastn size)
  "(ncycles lastn size)
Run ncycles & analyze last lastn cycles for torus of size."
...
(defun run (ncycles lastn nruns &optional (size 10) (ntags 1) (nstrategies 2) (cost 0.01) (benefit 0.03) 
...
Run ncycles & analyze last lastn cycles for nruns & other optional parameters."
  (setq *path* "/Users/christinaisaicu/Documents/0McGill/u3/FALL2018/PSYC315/ethnocentrism/code/10x10/1-tag/run-1000-100-10/")
  (seed-random)
  ...
  (do ((i 0 (1+ i)))
      ((= i nruns) (progn
                     (lists->file *strategies* (concatenate 'string *path* "strategies.txt"))
                     (lists->file *strategy-proportions* (concatenate 'string *path* "strategy-proportions.txt"))
                     (lists->file *behaviors* (concatenate 'string *path* "behaviors.txt"))
                     (lists->file *behavior-proportions* (concatenate 'string *path* "behavior-proportions.txt"))
                     'done))
    (run1 ncycles lastn size)
    (print i)))
...
top2 is critical value for most frequent vs next most frequent x^2."
  ...
  (do ((i 0 (1+ i))
      ...
      ((= i ncycles) (lists->file 
                      strategies 
                      (concatenate 'string *path* "strategies" (princ-to-string n) ".txt")))                       
    (immigrate)
    (all-interactions)
    (reproduction)
    (death)
...
(defun run-all-strategies (ncycles nruns overall top2 &optional (size 10) (ntags 1) (nstrategies 2) 
...
overall is critical value for overall x^2. 
top2 is critical value for most frequent vs next most frequent x^2."
  (setq *path* "/Users/christinaisaicu/Documents/0McGill/u3/FALL2018/PSYC315/ethnocentrism/code/10x10/1-tag/run-all-s-1000-10-11_35_6_64/")
  (seed-random)
  (set-parameters ntags nstrategies cost benefit base-ptr mutation-rate death-rate immigration-rate)
  (list-parameters)
  (do ((i 0 (1+ i)))
...
(defun run1plots (ncycles nth &optional (size 10) (ntags 1) (nstrategies 2) (cost 0.01) 
...
Run ncycles & plot every nth cycle for torus of size."
  (setq *path* "/Users/christinaisaicu/Documents/0McGill/u3/FALL2018/PSYC315/ethnocentrism/code/10x10/1-tag/run1plots-1000-100-10/")
  (seed-random)
  (set-parameters ntags nstrategies cost benefit base-ptr mutation-rate death-rate immigration-rate)
  (initialize-torus size)
  (do ((i 1 (1+ i))
      ...
      ((> i ncycles) (progn
                       (lists->file strategies
                                    (concatenate 'string *path* "strategies.txt"))
                       (lists->file (proportion-of4-strategies strategies)
                                    (concatenate 'string *path* "proportions.txt"))))
    (immigrate)
    (all-interactions)
    (reproduction)
    (death)
...
(run1plots 1000 100 10)
(run-all-strategies 1000 10 11.35 6.64)
(run 1000 100 10)
